...
"""
    ...
    Modulo de Interface Gráfica do PpyCACIC.
...
class GUI: 
    ...
    def createTray(self):
        ...
        #Montando exibicao do IP no ToolTip
        if cacic_dat.has_key('TcpIp.TE_IP'):
            self.statusIcon.set_tooltip(self.TITLE + ' v: ' + self.utils.getVersao()+'\n------------------------------\nIP: '+cacic_dat['TcpIp.TE_IP'])
...
class LogAtividades(GUI):
    ...
    def show(self):
        ...
        self.xml.get_widget('label_versao').set_text('Versão: ' + utils.getVersao())				
        # Label
...
class InfoGeral(GUI):
    ...
    def setTreeViewsValues(self):        
        ...
        if cacic_dat.has_key('Coletas.HOJE'):
            ...
            self.xml.get_widget('lb_cols').set_text('%s (%s)' % (lbl, 'Coletas Ainda Nao Realizadas!'))
